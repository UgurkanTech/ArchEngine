...
namespace ArchEngine.Core.Physics
{
    public class PhysicsCore 
    {
        ...
        public ConvexHullShape SetupFromMesh(Mesh mesh)
        {
            ...
            {
                var vertex0 = FromVector3D(vertices[i]);
                var vertex1 = FromVector3D(vertices[i + 1]);
                var vertex2 = FromVector3D(vertices[i + 2]);
        ...
        public static RigidBody AddMesh(Scene assimpScene, bool isStatic, Vector3 scale)
        {
            ...
            convexHullShape.LocalScaling = new Vector3(0.5f, 0.5f, 0.5f); // Set local scaling
            
            
            var obj2Transform = Matrix.Translation(0, 5, 0);
            obj2Transform.ScaleVector = scale;
            var obj2MotionState = new ArchMotionState(obj2Transform);
